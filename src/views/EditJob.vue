...
onMounted(async () => {
  try {
    ...
    if (response.data) {
      ...
      console.error(">>>> No data found for the given job ID.");
    }
  } catch (error) {
...
const handleUpdateJob = async (event) => {
  ...
  try {
    const { id } = route.params;
    const response = await axios.put(`/api/jobs/${id}`, editJob, {
      method: "PUT",
    });
    router.push(`/jobs/${response.data.id}`);
    toast.success("Updated a new job successfully");
  } catch (error) {
...
<template>
  <section class="bg-green-50">
    <div
      ...
      style="max-width: 500px; width: 100%; padding: 40px 0"
    >
      <div
